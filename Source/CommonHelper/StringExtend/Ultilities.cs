...
namespace CommonHelper
{
    public static class Ultilities
    {
        ...
        /// <summary>
        /// @description: lấy số tiền tố
        /// @author: duynn
        /// @since: 13/08/2018
        /// </summary>
        /// <param name="text"></param>
        /// <returns></returns>
        public static int GetPrefixNumber(this string text)
        {
            int result = 0;
            string tempStringNumber = string.Empty;
            if (!string.IsNullOrEmpty(text) && text.Any(char.IsDigit))
            {
                //nếu GHICHU chỉ có số
                bool hasAlpha = Regex.Matches(text.Trim(), @"[a-zA-Z]").Count > 0;
                if (hasAlpha)
                {
                    char[] arrChars = text.ToCharArray();
                    for (int x = 0; x < arrChars.Length; x++)
                    {
                        if (char.IsDigit(arrChars[x]))
                        {
                            tempStringNumber += arrChars[x].ToString();
                        }
                        else
                        {
                            break;
